...
/** Danh sách sản phẩm chỉ đọc cho operations: chỉ xem và mở trang biến thể (không sửa/xóa). */
export default function OpsProductListPage() {
    ...
    <div>
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Xem danh sách và biến thể — không chỉnh sửa catalog.</p>
        </div>
      </div>
      ...
      <form onSubmit={onSearch} className="mb-6 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        <div className="min-w-[200px] flex-1 space-y-1">
          <Label htmlFor="ops-q">Tìm kiếm</Label>
          <Input id="ops-q" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tên, SKU…" />
        </div>
        <div className="w-44 space-y-1">
          <Label htmlFor="ops-cat">category_id</Label>
          <Input id="ops-cat" value={categoryId} onChange={(e) => setCategoryId(e.target.value)} placeholder="Mongo ObjectId" />
        </div>
                ...
                return (
                  <tr key={id || (p.slug as string)} className="border-b border-slate-100 hover:bg-slate-50/80">
                    ...
                    <td className="px-4 py-3 text-right">
                      {id ? (
                        <Link
                          to={`/admin/ops/catalog/products/${encodeURIComponent(id)}/variants`}
      ...
      <div className="mt-4 flex gap-2">
        <Button type="button" variant="outline" disabled={page <= 1 || loading} onClick={() => setPage((pg) => pg - 1)}>
          Trang trước
        </Button>
        <Button type="button" variant="outline" disabled={loading || list.length < limit} onClick={() => setPage((pg) => pg + 1)}>
          Trang sau
        </Button>
